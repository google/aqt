...
def _lp_dev(
  ...
  if lp_order == 2:
    ...
    return tf.sqrt(
        divide_fn(sum_of_lp_vals.read_value(), sum_of_ones.read_value())
    )
  else:
    # TODO(b/205769820): Make sure if the output of pow op below is
    # numerically valid
    return divide_fn(sum_of_lp_vals.read_value(), sum_of_ones.read_value()) ** (
        1.0 / lp_order
    )
...
def _sum_of_vals(
  ...
  return _reduce_fn(stats_config, x, weight)
...
def _bound(
  ...
  if calibration_config.lp_dev_coeff:
    lp_dev = divide_fn(sum_of_lp_vals, sum_of_ones)
...
def dynamic_bound(
  ...
  """Comput the upper bound on input tensor values dynamically."""
  config.validate(x.shape.as_list())
  stats_shape = _stats_shape(config, x.shape.as_list())
...
class Stats:
  ...
  def __init__(
    ...
    self.stats_shape = _stats_shape(self._config, self._data_shape)
  ...
  def bound(  #
    ...
    return _bound(
        ...
        self.stats_shape,
        self._sum_of_ones.read_value(),
        self._max_of_abs_vals.read_value(),
...
def _config_case(
    tensor_configs: Iterable[aqt_config.AqtTensorConfig],  #
  ...
  assert tensor_configs, 'There must be at least one config.'

  def make_case(config):
    pred = is_config_active(config, event_count)
    return pred, lambda: case_fn(config)

  cases = [make_case(c) for c in tensor_configs]
...
class TensorQuantizer:
  ...
  def __init__(
    ...
    with tf.variable_scope(name):
      if not self.config.stats_config.dynamic:
        self._stats = Stats(
        ...
        # We intentionally initialize scale to zero to fail loudly if someone
        # uses a parameter such as scale without properly update()-ing it.
        self._scale = get_variable(
            'scale', self._stats.stats_shape, tf.float32, 0
        )
        # Save the inverse scale so that we don't recompute it at inference
        # time.
        ...
        self.config.stats_config.validate(self.data_shape)
  ...
  def validate_static(self, msg: str):
    if self.config.stats_config.dynamic:
      raise ValueError('Cannot store statistics for dynamic quantization due '
                       f'to possible unknown dimensions. {msg}'
                       )
  ...
  def calibration_variables(self) -> Dict[str, tf.Variable]:
    ...
    if self.config.stats_config.dynamic:
      # if dynamic no calibration variables are stored
      return {}
    return {
        'scale': self._scale,
  ...
  def clip_range(self) -> tf.Tensor:
    ...
    self.validate_static('Use dynamic_clip_range instead.')
  ...
  def dynamic_clip_range(
    ...
    """Returns the tensor clip range or zeros if no int config is active."""
    assert self.config.stats_config.dynamic
    ...
    def case_fn(config: aqt_config.AqtTensorConfig) -> tf.Tensor:
      if isinstance(config.quant_config, aqt_config.FloatConfig):
        stats_shape = _stats_shape(
            self.config.stats_config, sample.shape.as_list()
        )
        return tf.zeros(stats_shape)
  ...
  def _maybe_fallback_to_ones(
    ...
    with tf.variable_scope('to_quant'):
      scale = tf.where_v2(  #
          should_scale, scale, tf.ones_like(scale))
  ...
  def _get_quant_scale(self, train: bool) -> tf.Tensor:
    ...
    self.validate_static('_get_dynamic_quant_scale to calculate quant scale '
                         'on the fly.')
  ...
  def _fresh_dynamic_scale(
    ...
    if isinstance(tensor_config.quant_config, aqt_config.FloatConfig):
      ...
      nan = tf.constant(
          float('nan'),
          sample.dtype,
          _stats_shape(self.config.stats_config, sample.shape.as_list()),
      )
      return nan, nan

    x_bound = dynamic_bound(
