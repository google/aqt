...
def _make_dot_general_raw(cfg: config.DotGeneralRaw):
  ...
  def dot_general_raw(
    ...
    if cfg.dg_accumulator_dtype == jnp.int32:
      int_dtype = utils.int_dtype
      assert lhs_qin.dtype in int_dtype and rhs_qin.dtype in int_dtype, dtype_ms

    if cfg.lhs.dequant_mode == config.DequantMode.OTHER_INPUT:
      assert rhs_qin.dtype in utils.float_dtype
    ...
    if cfg.rhs.dequant_mode == config.DequantMode.OTHER_INPUT:
      assert lhs_qin.dtype in utils.float_dtype
