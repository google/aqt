...
def _cast(
    x: aqt_tensor.QTensor, dtype: Optional[jnp.dtype], fake_quant: bool
) -> jnp.ndarray:
  if fake_quant:
    output = x.dequant()
  else:
    output = x.qvalue.astype(dtype) if dtype is not None else x.qvalue
  return output
...
def _make_dot_general_raw(cfg: config.DotGeneralRaw):
  ...
  def dot_general_raw(
    ...
    lhs_qin = _cast(lhs_qt, lhs_cast_dtype, cfg.lhs.use_fake_quant)
    rhs_qin = _cast(rhs_qt, rhs_cast_dtype, cfg.rhs.use_fake_quant)
