...
class CNN(nn.Module):
  ...
  lhs_quant_mode: aqt_flax.QuantMode = aqt_flax.QuantMode.TRAIN
  rhs_quant_mode: aqt_flax.QuantMode = aqt_flax.QuantMode.TRAIN
...
def calibrate(state, train_ds, calibration_steps):
  """Calibrate."""
  cnn_calibrate = CNN(
      bn_use_stats=False,
      aqt_cfg=state.cnn_eval.aqt_cfg,
      # Both side should be calibrated.
      lhs_quant_mode=aqt_flax.QuantMode.CALIBRATE,
      rhs_quant_mode=aqt_flax.QuantMode.CALIBRATE,
  )

  # Run calibration examples for several batches to gather the statistics info.
  batch_size = 128
  rng = jax.random.PRNGKey(1)
  train_ds_size = len(train_ds['image'])
  steps_per_epoch = train_ds_size // batch_size

  # Normally around < 10 calibration steps should be enough with batch size 128.
  assert calibration_steps < steps_per_epoch

  perms = jax.random.permutation(rng, len(train_ds['image']))
  perms = perms[: calibration_steps * batch_size]  # skip incomplete batch
  perms = perms.reshape((calibration_steps, batch_size))

  model_calibrated = state.model

  for perm in perms:
    batch_images = train_ds['image'][perm, ...]

    _, model_calibrated = cnn_calibrate.apply(
        model_calibrated,
        batch_images,
        rngs={'params': jax.random.PRNGKey(0)},
        mutable=True,
...
def serve(state):
  ...
  train_ds, test_ds = get_datasets()
  model_calibrated = calibrate(state, train_ds, calibration_steps=5)
  state = state.replace(model=model_calibrated)
...
def main(argv):
  ...
  aqt_cfg.fwd.lhs.quantizer.calib_shared_axes = 'per_tensor'
  state = train_and_evaluate(
      num_epochs=2, workdir='/tmp/aqt_mnist_example', aqt_cfg=aqt_cfg
  )

  state.cnn_train.aqt_cfg.fwd.lhs.quantizer.calibration = (
      aqt_state_updator.StaticRangeCalibration(moving_average_weight=0.9)
  )
  state.cnn_train.aqt_cfg.fwd.rhs.quantizer.calibration = (
      aqt_state_updator.StaticRangeCalibration(moving_average_weight=0.9)
  )
  state.cnn_eval.aqt_cfg.fwd.lhs.quantizer.calibration = (
      aqt_state_updator.StaticRangeCalibration(moving_average_weight=0.9)
  )
  state.cnn_eval.aqt_cfg.fwd.rhs.quantizer.calibration = (
      aqt_state_updator.StaticRangeCalibration(moving_average_weight=0.9)
  )
