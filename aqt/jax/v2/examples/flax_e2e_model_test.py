...
class MnistTest(parameterized.TestCase):
  ...
  def test_mnist_training(self, configs, bits):
    ...
    target_loss = {
        8: {
            ...
            "TPU v5 lite": [3.200393676757812500000000000000],
        },
        4: {
            ...
            "TPU v5 lite": [2.302415609359741210937500000000],
        },
    }
  ...
  def test_mnist_delayed_scaling_calibration(self, bits):
    ...
    calibration_cls = functools.partial(
        transformer_engine_calibration.DelayedScalingCalibration,
    ...
    utils.test_pprint_eq(expected_trained_pytree, trained_pytree["fp8_stats"])
    ...
    self.assertEqual(
        jnp.sum(
            state.model["fp8_stats"]["Dense_0"]["AqtDotGeneral_0"][
                "DelayedScalingCalibration_0"
            ]["amax_history"][:] != 0
        ).item(),
        num_train_steps + 1,
