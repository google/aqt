...
def quant(
  ...
  assert len(qx.scale) == 1, 'Only one scale factor is supported on pallas.'

  def scale_index_map(*args):
    index = block_spec.index_map(*args)
    # there is only one block on calibration axes.
    index = [
        0 if ax in calibration_axes else ind
        for ax, ind in enumerate(index)
    ]
    return index
  ...
  qx_block_spec = dataclasses.replace(
      ...
      qvalue=block_spec,
      scale=[
          pl.BlockSpec(index_map=scale_index_map, block_shape=scale_block_shape)
      ],
  )
